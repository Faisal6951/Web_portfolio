...
type FormState = 'idle' | 'transmitting' | 'sent' | 'error' | 'invalid_email';
...
export function TransmissionPage() {
  ...
  const handleChange = (
    ...
    if (formState === 'error' || formState === 'invalid_email') {
      setFormState('idle');
      setErrorMessage('');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidEmail(values.channel)) {
    ...
    if (
      ...
      setFormState('error');
      setErrorMessage('All fields are required. Fill in the blanks, Agent.');
      return;
    }
    ...
    try {
      const response = await fetch('https://formspree.io/f/xjgdnrwz', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          name: values.callsign.trim(),
          email: values.channel.trim(),
      ...
      if (response.ok) {
        setFormState('sent');
        setValues({ callsign: '', channel: '', subject: '', message: '' });
      } else {
        const formspreeError: string =
          data?.errors
            ?.map((err: { message: string }) => err.message)
            .join(', ') || 'Transmission failed. Try again.';
        setErrorMessage(formspreeError);
        setFormState('error');
      }
    } catch {
      setErrorMessage('Connection lost. Check your network and retry.');
      setFormState('error');
    }
  };
                  ...
                  >
                    {FORM_FIELDS.map((field) => (
                      <div key={field.id} className='flex flex-col gap-2'>
                        ...
                        >
                          {field.id === focused ||
                          values[field.id as keyof FormValues]
                            ? `▸ ${field.label}`
                            : `○ ${field.label}`}
                        </label>
                        <div
                    ...
                    {(formState === 'error' ||
                      ...
                      <motion.p
                        ...
                        className='font-mono text-[10px] text-blood-red tracking-wider'
                      >
                        ✕ {errorMessage}
                      </motion.p>
                    )}
